...
if [ $# -ge 0 ] && [ $# -le 1 ]; then
    case "$1" in
        "")
            ...
            str=" con all'interno alcuni file. All'inteno di questa ti chiedo di creare una cartella "
            print_animated "$str" "$WHITE"
            print_animated "playlists" "$YELLOW"
            str=". Dentro questa cartella dovrai creare due playlist chiamate rispettivamente: "
            print_animated "$str" "$WHITE"
            ...
            print_animated "$BOTIFY" "$GREEN"
            str=": Solo per essere chiari... le playlist che creerai dovranno essere inserite nella cartella di nome "
            print_animated "$str" "$WHITE"
            print_animated "playlists" "$YELLOW"
            print_animated "." "$WHITE"
